    ...
    return (
        <main className="max-w-2xl mx-auto p-6">
            ...
            {(!coffees || coffees.length === 0) && (
                <div className="text-gray-400 text-center py-10">Aucun café référencé pour l’instant…</div>
            )}
                    ...
                    <Card key={cafe.id} className="transition border">
                        <CardHeader>
                            <CardTitle className="flex items-center gap-2">
                                {cafe.name}
                                {cafe.origin && (
                                    <Badge variant="secondary" className="ml-2">{cafe.origin}</Badge>
                                )}
                            </CardTitle>
                            {cafe.description && (
                                <div className="text-xs text-gray-500 mt-1">{cafe.description}</div>
                            )}
                        </CardHeader>
                        <CardContent>
